...
if [ -n "$WHAT" ]; then
	if ["$WHAT" = "README"] || ["$WHAT" = "ChangeLog"] || ["$WHAT" = "usr/include/"] || ["$WHAT" = "usr/share/backgrounds/"] || ["$WHAT" = "usr/share/pixmaps/"] || ["$WHAT" = "usr/share/man/"]; then #Check if has been selected any option
		echo "0"
		dir=$(zenity --title="Directorer" --text "Inroduce a directory" --entry) #Ask for the directory
